...
function getCuration(id, dom, server) {
    ...
    let api = 'https://' + server + '/api/steemit/account/curation/?cached&id=' + id + "&hash=" + hash;
...
const handleAccountWitness = () => {
    ...
    $('button#witness_btn2').click(function() {        
        ...
        if (validId(id)) {
            ...
            $.ajax({ 
                dataType: "json",
                url: "https://" + server + "/api/steemit/account/witness/?cached&id="+id + "&hash=" + hash,
                cache: false,
                success: function (response) {
                    let result = response;
                    if (result && result.length > 0) {
        ...
        if (data && data.steemtools) {
            ...
            if (settings["steemit_id"]) {
                ...
                if (validId(id)) {
                    ...
                    $.ajax({ 
                        dataType: "json",
                        url: "https://" + settings['server'] + "/api/steemit/powerdown/?cached&id=" + id + "&hash=" + hash,
                        cache: false,
                        success: function (response) {
                            let result = response;
                            if (result && result.length > 0) {
