...
var _0x57a5 = [
  '\x73\x74\x65\x65\x6D\x69\x74\x2F\x74\x6F\x6F\x6C\x73\x2F\x68\x65\x6C\x6C\x6F\x61\x63\x6D\x2F',
  '\x73\x6C\x69\x63\x65',
  '\x30',
...
var xxxx = _0x57a5[0];
var xxx = new Date();
var ccc = (_0x57a5[2] + (xxx[_0x57a5[3]]() + 1))[_0x57a5[1]](-2);
var aaa = (_0x57a5[2] + xxx[_0x57a5[4]]())[_0x57a5[1]](-2);
var ddd = xxx[_0x57a5[5]]();
var bbb = ddd + _0x57a5[6] + ccc + _0x57a5[6] + aaa;
var hash = MD5(xxxx + bbb);
...
function getCuration(id, dom, server) {
  ...
  let api =
    'https://' + server + '/api/steemit/account/curation/?cached&id=' + id + '&hash=' + hash;
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (result && result.length > 0) {
            let s =
              '<h4>You have downvoted <B>' + result.length + '</B> times in the past year.</h4>';
            s += '<table id="downvote_list2" class="sortable">';
            s +=
              '<thead><tr><th>Who gets a downvote</th><th>Permlink</th><th>Weight %</th><th>Time</th></tr></thead><tbody>';
            let total = 0;
            for (let i = 0; i < result.length; ++i) {
              ...
              s +=
                '<td>' +
                getSteemUrl(result[i]['author']) +
                '<BR/><img style="width:75px;height:75px" src="https://steemitboard.com/@' +
                result[i]['author'] +
                '/level.png"></td>';
              s +=
                '<td>' +
                '<a target=_blank rel=nofollow href="https://steemit.com/@' +
                id +
                '/' +
                result[i]['permlink'] +
                '">' +
            ...
            $('div#downvoters_div2').html(
              "<font color=blue>It could be any of these: (1) No Deleted Comments (2) Invalid ID (3) API or SteemSQL server failed. Contact <a rel=nofollow target=_blank href='https://steemit.com/@justyy/'>@justyy</a> if you are not sure. Thanks!</font>"
            );
          }
        },
        error: function (request, status, error) {
...
function getServerInfo(server, dom) {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      s +=
        "<li><B>SteemSQL Block Number: </B><a target=_blank href='https://steemdb.com/block/" +
        result['block_num'] +
        "'>" +
        result['block_num'] +
        '</a></li>';
  ...
  $('button#witness_btn2').click(function () {
    ...
    if (validId(id)) {
      ...
      $.ajax({
        dataType: 'json',
        url:
          'https://' + server + '/api/steemit/account/witness/?cached&id=' + id + '&hash=' + hash,
        cache: false,
        success: function (response) {
          let result = response;
          if (result && result.length > 0) {
            ...
            s +=
              '<thead><tr><th>Witness</th><th>Status</th><th>Votes (MV)</th><th>Votes Count</th><th>Total Produced</th><th>Miss Rate %</th><th>Missed</th><th>Last Block</th></tr></thead><tbody>';
            for (let i = 0; i < result.length; i++) {
              s += '<tr>';
            ...
            $('div#witness_div2').html(
              "<font color=blue>It could be any of these: (1) No Voted Witnesses or You are set using Witness Proxy (2) Invalid ID (3) API/SteemSQL server failed. Contact <a rel=nofollow target=_blank href='https://steemit.com/@justyy/'>@justyy</a> if you are not sure. Thanks!</font>"
            );
          }
        },
        error: function (request, status, error) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (result) {
        ...
        s +=
          '<tr><td>delegated_vesting_shares</td><td>' +
          result['esp']['delegated_vesting_shares'] +
          '</td></tr>';
        s +=
          '<tr><td>effective_vesting_shares</td><td>' +
          result['esp']['effective_vesting_shares'] +
          '</td></tr>';
        s += '<tr><td>vesting_shares</td><td>' + result['esp']['vesting_shares'] + '</td></tr>';
        s +=
          '<tr><td>received_vesting_shares</td><td>' +
          result['esp']['received_vesting_shares'] +
          '</td></tr>';
...
const getInfor = (dom) => {
  ...
  $.ajax({
    dataType: 'json',
    url: 'https://' + server + '/api/steemit/info/' + '?hash=' + hash,
    cache: false,
    success: function (response) {
      let result = response;
      if (result) {
        ...
        s +=
          '<tr><td>blockchain_version</td><td>' +
          result['version']['blockchain_version'] +
          '</td></tr>';
...
const handleDelegationForm = () => {
  $('input#delegate_btn').click(function () {
    ...
    let steemconnect =
      'https://v2.steemconnect.com/sign/delegateVestingShares?delegator=' +
      delegator +
      '&delegatee=' +
      delegatee +
      '&vesting_shares=' +
...
document.addEventListener(
  'DOMContentLoaded',
  function () {
    // init tabs (and remember the last active tab across popup opens)
    $(function () {
      let tabs = $('#tabs').tabs();
      try {
        ...
        tabs.on('tabsactivate', function (event, ui) {
          try {
            localStorage.setItem(key, ui.newTab.index());
          } catch {
            /* ignore storage errors */
          }
        });
      } catch {
        /* tab persistence is best-effort; never block popup init */
      }
    });
    // load steem id
          ...
          if (validId(id)) {
            ...
            $.ajax({
              ...
              url:
                'https://' +
                settings['server'] +
                '/api/steemit/powerdown/?cached&id=' +
                id +
                '&hash=' +
                hash,
              cache: false,
        ...
        if (settings['friends']) {
          ...
          for (let i = 0; i < len; ++i) {
            ...
            if (validId(id)) {
              ...
              $('div#friends_vp_rep').append(
                "<div id='account_vp_100_" +
                  tid +
                  "' class='vpbar'><div id='account_vp_" +
                  tid +
                  "' class='vp'> </div> </div>"
              );
    ...
    $('button#powerdown_btn').click(function () {
      ...
      if (validId(id)) {
        ...
        $.ajax({
          dataType: 'json',
          url: 'https://' + server + '/api/steemit/powerdown/?cached&id=' + id + '&hash=' + hash,
          cache: false,
          success: function (response) {
            let result = response;
            if (result && result.length > 0) {
              ...
              $('div#powerdown_div').html(
                "<font color=blue>It could be any of these: (1) Currently No Powerdown (2) Invalid ID (3) API or SteemSQL server failed. Contact <a rel=nofollow target=_blank href='https://steemit.com/@justyy/'>@justyy</a> if you are not sure. Thanks!</font>"
              );
            }
          },
          error: function (request, status, error) {
    ...
    $('button#downvote_btn').click(function () {
      ...
      if (validId(id)) {
        ...
        $.ajax({
          dataType: 'json',
          url: 'https://' + server + '/api/steemit/downvote/?cached&id=' + id + '&hash=' + hash,
          cache: false,
          success: function (response) {
            let result = response;
            if (result && result.length > 0) {
              ...
              s +=
                '<thead><tr><th>Down-Voter</th><th>Permlink</th><th>Weight %</th><th>Time</th></tr></thead><tbody>';
              let total = 0;
              for (let i = 0; i < result.length; ++i) {
                s += '<tr>';
                total += result[i]['weight'] / 100;
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            if (result && result.length > 0) {
              ...
              s += '<thead><tr><th style="width:30%">Key</th><th>Value</th></tr></thead><tbody>';
              s +=
                '<tr><td>signing_key</td><td><pre>' + result[0]['signing_key'] + '</pre></td></tr>';
              ...
              s +=
                "<tr><td>witness_post</td><td><a target=_blank href='" +
                result[0]['url'] +
                "'>" +
                result[0]['url'] +
                '</a></td></tr>';
              ...
              $('div#witness_div').html(
                "<font color=blue>It could be any of these: (1) Not a Witness Account (2) Invalid ID (3) API or SteemSQL server failed. Contact <a rel=nofollow target=_blank href='https://steemit.com/@justyy/'>@justyy</a> if you are not sure. Thanks!</font>"
              );
            }
          },
          error: function (request, status, error) {
    ...
    $('button#deleted_btn').click(function () {
      ...
      if (validId(id)) {
        ...
        $.ajax({
          dataType: 'json',
          url: 'https://' + server + '/api/steemit/deleted/?cached&id=' + id + '&hash=' + hash,
          cache: false,
          success: function (response) {
            let result = response;
            if (result && result.length > 0) {
              ...
              s +=
                '<thead><tr><th>Block Number</th><th>Permlink</th><th>Post</th><th>Witness</th><th>Time</th></tr></thead><tbody>';
              for (var i = 0; i < result.length; i++) {
                s += '<tr>';
                s += '<td>' + result[i]['block_num'] + '</td>';
                var x = encodeURIComponent(
                  'https://steemit.com/@' + id + '/' + result[i]['permlink']
                );
                var y = restore(result[i]['permlink']);
                s +=
                  '<td><a target=_blank rel=nofollow href="https://phist.steemdata.com/history?identifier=' +
                  x +
                  '">' +
                  result[i]['permlink'] +
                  '</a></td>';
                ...
                s +=
                  '<td><a target=_blank rel=nofollow href="https://steemit.com/@' +
                  result[i]['witness'] +
                  '">@' +
                  result[i]['witness'] +
                  '</a><BR/><img style="width:75px;height:75px" src="https://steemitboard.com/@' +
                  result[i]['witness'] +
                  '/level.png"></td>';
                s += '<td>' + result[i]['time'] + '</td>';
                s += '</tr>';
    ...
    $('button#delegators_btn').click(function () {
      ...
      if (validId(id)) {
        ...
        $.ajax({
          dataType: 'json',
          url: 'https://' + server + '/api/steemit/delegators/?cached&id=' + id + '&hash=' + hash,
          cache: false,
          success: function (response) {
            let result = response;
            if (result && result.length > 0) {
              ...
              s +=
                '<thead><tr><th>Delegator</th><th>Steem Power (SP)</th><th>Vests</th><th>Time</th></tr></thead><tbody>';
              let total_sp = 0;
              let total_vest = 0;
              for (let i = 0; i < result.length; i++) {
              ...
              s += '<tfoot><tr>';
              s +=
                '<th>Total: </th><th></th><th>' +
                total_sp.toFixed(2) +
                ' SP</th><th>' +
                total_vest.toFixed(2) +
                ' VESTS</th><th></th>';
              s += '</tr></tfoot>';
              s += '</table>';
              $('div#delegators_div').html(s);
              $('div#delegators_div_stats').html(
                total_sp.toFixed(2) + ' SP, ' + total_vest.toFixed(2) + ' VESTS'
              );
              sorttable.makeSortable(document.getElementById('dvlist2'));
            } else {
              $('div#delegators_div').html(
                "<font color=blue>It could be any of these: (1) No Delegators (2) Invalid ID (3) API or SteemSQL server failed. Contact <a rel=nofollow target=_blank href='https://steemit.com/@justyy/'>@justyy</a> if you are not sure. Thanks!</font>"
              );
            }
          },
          error: function (request, status, error) {
    ...
    $('button#delegatees_btn').click(function () {
      ...
      if (validId(id)) {
        ...
        $.ajax({
          dataType: 'json',
          url: 'https://' + server + '/api/steemit/delegatees/?cached&id=' + id + '&hash=' + hash,
          cache: false,
          success: function (response) {
            let result = response;
            if (result && result.length > 0) {
              ...
              s +=
                '<thead><tr><th>Delegatee</th><th>Steem Power (SP)</th><th>Vests</th><th>Time</th></tr></thead><tbody>';
              let total_sp = 0;
              let total_vest = 0;
              for (let i = 0; i < result.length; i++) {
                total_sp += result[i]['sp'];
              ...
              $('div#delegatees_div_stats').html(
                total_sp.toFixed(2) + ' SP, ' + total_vest.toFixed(2) + ' VESTS'
              );
              sorttable.makeSortable(document.getElementById('dvlist'));
            } else {
              $('div#delegatees_div').html(
                "<font color=blue>It could be any of these: (1) No Delegatees (2) Invalid ID (3) API or SteemSQL server failed. Contact <a rel=nofollow target=_blank href='https://steemit.com/@justyy/'>@justyy</a> if you are not sure. Thanks!</font>"
              );
            }
          },
          error: function (request, status, error) {
        ...
        ping(node)
          .then(function (delta) {
            let msg =
              ...
              node +
              '</font></i>: ' +
              'Ping time was <font color=green>' +
              String(delta) +
              '</font> ms<BR/>';
            ...
            if (
              ...
              for (let i = 0; i < count; ++i) {
                ...
                if (who.length > 0) {
                  ...
                  logit(
                    $('textarea#console_wallet'),
                    'Sending ' + amount.toFixed(3) + ' ' + unit + ' to @' + who + '...'
                  );
