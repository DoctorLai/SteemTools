...
- A **GitHub Pages** deployment (`.github/workflows/deploy.yml`) that publishes a
  live preview of the popup, backed by a lightweight `chrome.*` shim
  (`js/chrome-shim.js`) so it runs outside the extension.
